...
public class AssetSetter {
	...
	public void setObject() {
		for (int i = 0; i < fileNames.size(); i++) {
			...
			switch (name[0]) {
			case "Bit":
				gp.getSuperObject()[i] = new Bit();
				break;
			case "Coin":
				gp.getSuperObject()[i] = new Coin();
				break;
			case "Door":
				gp.getSuperObject()[i] = new Door();
				break;
			case "Fire":
				gp.getSuperObject()[i] = new Fire();
				break;
			case "Key":
				gp.getSuperObject()[i] = new Key();
				break;
			case "Trap":
				gp.getSuperObject()[i] = new Trap("Trap" + name[1]);
				break;
			case "DeadBox":
				gp.getSuperObject()[i] = new DeadBox();
				break;
			case "Spike":
				gp.getSuperObject()[i] = new Spike("Spike" + name[1]);
				break;
			case "EndGame":
				gp.getSuperObject()[i] = new EndGame();
				break;
			}
